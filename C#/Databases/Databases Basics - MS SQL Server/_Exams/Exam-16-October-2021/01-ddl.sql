...
(
	...
	CONSTRAINT CHK_LengthBetween10And25 CHECK ([Length] BETWEEN 10 AND 25),
	[RingRange] DECIMAL(3,2) NOT NULL,
	CONSTRAINT CHK_RingRange_BetweenSizes CHECK ([RingRange] BETWEEN 1.5 AND 7.5),
)
...
SELECT 
	...
	Country,
	ZIP,
	CONCAT('$',notFormated.[CigarPrice]) AS [CigarPrice]
FROM
(SELECT 
	temp.FullName,
	temp.Country,
	temp.ZIP,
	MAX(temp.PriceForSingleCigar) AS [CigarPrice]
		FROM (
...
FROM Clients AS c
JOIN Addresses AS ad ON c.AddressId = ad.Id
JOIN ClientsCigars AS cc ON c.Id = cc.ClientId
JOIN Cigars AS ci ON cc.CigarId = ci.Id
WHERE ISNUMERIC(ad.ZIP) = 1 ) AS Temp
GROUP BY temp.[FullName], temp.Country, temp.Zip) AS notFormated
...
SELECT 
	temp.LastName,
	temp.CiagrLength,
	CEILING(temp.CiarRingRange) AS [CiarRingRange]
	FROM
(
SELECT c.LastName,
		AVG(s.Length) AS [CiagrLength],
		AVG(s.RingRange) AS [CiarRingRange]
FROM Clients AS c
RIGHT JOIN ClientsCigars AS cc ON c.Id = cc.ClientId
JOIN Cigars AS ci ON cc.CigarId = ci.Id
JOIN Sizes AS s ON ci.SizeId = s.Id
GROUP BY LastName ) AS temp
ORDER BY temp.[CiagrLength] DESC

-- 11: Client with Cigars

CREATE FUNCTION udf_ClientWithCigars(@name NVARCHAR(25))
RETURNS INT
AS
BEGIN
	DECLARE @Output INT = (
...
--SELECT dbo.udf_ClientWithCigars('dsdgsd') AS [Test]
